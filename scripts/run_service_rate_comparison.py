#!/usr/bin/env python3
"""Run service-rate-control debug configs and aggregate comparison summaries."""
...
def main() -> int:
    ...
    run_dir = create_run_dir(
        ...
        config["experiment"].get("name", "service_rate_comparison"),
    )
    ...
    for row in rows:
        print(
            ...
            f"threshold_high={row['first_state_p_high_at_least_threshold']} "
...
def _run_one_method(method: str, config_path: Path) -> dict:
    ...
    return _comparison_row(method, config_path, child_run_dir, summary, runtime)
...
def _comparison_row(
    ...
    policy_grid = summary.get("policy_grid", {})
    return {
        "method": method,
        ...
        "boundary_hit_fraction_mean": evaluation.get("boundary_hit_fraction_mean"),
        "tail_fraction_mean": evaluation.get("tail_fraction_mean"),
        "first_state_p_high_at_least_threshold": policy_grid.get(
            "first_state_p_high_at_least_threshold"
        ),
        "first_state_p_medium_at_least_threshold": policy_grid.get(
