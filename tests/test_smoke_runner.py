...
class SmokeRunnerTests(unittest.TestCase):
    ...
    def test_routing_amq_smoke_config_runs(self):
        root = Path(__file__).resolve().parents[1]
        completed = subprocess.run(
            [
                sys.executable,
                str(root / "scripts" / "run_experiment.py"),
                "--config",
                str(root / "configs" / "routing_amq_smoke.yaml"),
            ],
            cwd=root,
            text=True,
            stdout=subprocess.PIPE,
        ...
        self.assertTrue((run_dir / "q_diagnostic.jsonl").exists())
        summary = json.loads((run_dir / "summary.json").read_text(encoding="utf-8"))
        self.assertEqual(summary["benchmark"], "routing")
        self.assertEqual(summary["feature_set"], "basic")
        self.assertIn("policy_inspection", summary)
        self.assertIn("policy_comparison", summary)
        self.assertIn("q_diagnostic", summary)
    ...
    def test_routing_nnq_smoke_config_runs(self):
        root = Path(__file__).resolve().parents[1]
        completed = subprocess.run(
            [
                sys.executable,
                str(root / "scripts" / "run_experiment.py"),
                "--config",
                str(root / "configs" / "routing_nnq_smoke.yaml"),
            ],
            cwd=root,
            text=True,
            stdout=subprocess.PIPE,
